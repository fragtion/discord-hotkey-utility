...
The `mute_button` and `deafen_button` values refer to the byte index in the raw HID input report. Index `1` is the most common for the primary button on simple devices. You can identify the correct index by running the Windows Game Controllers control panel applet (`joy.cpl`) and checking the properties of your controller.
